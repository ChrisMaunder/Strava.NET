...
namespace Strava.Http
{
    ...
    public static class WebRequest
    {
        ...
        public static string SendGet(Uri uri)
        {
            ...
            Task<WebResponse> task = Task.Run(async () => await request.GetResponseAsync());
            ...
            using (response)
            {
                using (Stream responseStream = response.GetResponseStream())
                {
                    ...
                    {
                        ResponseReceived?.Invoke(null, new ResponseReceivedEventArgs(response));

                        //Getting the Strava API usage data.
                        string usage = response.Headers["X-RateLimit-Usage"];
